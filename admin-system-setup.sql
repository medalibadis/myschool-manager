...
CREATE POLICY "Allow public read access for active admins" ON admins FOR SELECT USING (is_active = true);
CREATE POLICY "Allow superuser full access" ON admins FOR ALL USING (role = 'superuser');
...
CREATE POLICY "Allow admins to manage their own sessions" ON admin_sessions FOR ALL USING (true);
