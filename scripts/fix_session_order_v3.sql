-- Fix Session Order After Rescheduling (Version 3)
-- This script handles NOT NULL constraints by using temporary negative numbers
...
-- Step 3: Set all existing session numbers to temporary negative values to avoid conflicts
-- (We can't use NULL because of NOT NULL constraint)
DO $$ 
BEGIN
    UPDATE sessions SET session_number = -id::integer WHERE session_number IS NOT NULL OR session_number IS NULL;
    RAISE NOTICE 'Set all session numbers to temporary negative values';
END $$;

-- Step 4: Assign proper session numbers to all sessions based on their current date order
DO $$ 
DECLARE 
    group_record RECORD;
